...
export async function GET(request: NextRequest) {
  try {
    ...
    if (campaignId > 0) {
      ...
      customRecords = context.homebrew.communities.map((item) => ({
        ...item,
        campaignId,
      }));
    }

    const records = [...toOfficialCommunityRecords(), ...customRecords].filter((record) =>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const context = parseCampaignHomebrewContext(campaign.description);
    const record = {
      id: createBuilderEntityId("community"),
    ...
    await persistCampaignHomebrew(
      ...
      {
        ...context.homebrew,
        communities: [...context.homebrew.communities, record],
      }
    );
