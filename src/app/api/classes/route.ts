...
import { toOfficialClassRecords, type HomebrewEntityRecord } from "@/lib/homebrew-library";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (campaignId > 0) {
      ...
      customRecords = context.homebrew.classes.map((item) => ({
        ...item,
        campaignId,
      }));
    }

    const records = [...toOfficialClassRecords(), ...customRecords].filter((record) =>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const context = parseCampaignHomebrewContext(campaign.description);
    const record = {
      id: createBuilderEntityId("class"),
    ...
    await persistCampaignHomebrew(
      ...
      {
        ...context.homebrew,
        classes: [...context.homebrew.classes, record],
      }
    );
